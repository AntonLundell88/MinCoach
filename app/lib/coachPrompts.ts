...
// Rösten och språket kommer utan gymscenen, besluten och sifferregeln
// (2026-09-15), se coachVoice.ts. Med "du står bredvid, du ser setet hända"
// skrev lobbyn som om eleven stod i gymmet, och fragmenten gav telegramstil:
// "Underkropp idag — fint."
...
const SET_COACH_INSTRUCTION = [
  ...
  // säger redan att gym kan skilja. Med stycket drog coachen in gymmet i 5 av
  // 6 PB-svar, utan det i 0 av 6. En lägre vikt förklarades med gymmet i 6 av
  // 6 med stycket och 5 av 6 utan (mätt 2026-09-14).
  "- otherGymReference: finns när vi inte vet var vikten ligger för den här övningen på det aktuella gymmet än (få gånger kört här, eller länge sen sist) — visar vad som faktiskt loggades senast på ett ANNAT gym (gymName, vikt, reps, RIR).",
...
export function buildCoachProgramPromptPayload(
  ...
  // Här stod exemplet "Krysset vid vadpress i Pass C gör jobbet." efter
  // pekaren till knapparna. Coachen skrev den meningen, med övning och pass
  // utbytta, i 6 av 6 svar som pekade dit (mätt 2026-09-14).
  const instruction =
    `Svara på det användaren faktiskt frågade eller oroar sig för — det kommer alltid först, som en erfaren coach som lyssnat på precis det. Skriv naturlig text — ingen JSON, ingen markdown, inga rubriker. Börja aldrig svaret med att peka ut ett pass eller en övning som "bäst" om det inte var det som frågades; det läser som att du svarar på fel fråga. Om ditt resonemang landar i att en specifik övning bör bytas ut, tas bort eller behållas: säg det naturligt, och lägg till en kort pekare i slutet till var i appen det görs — knapparna vid varje övning i respektive pass. Om frågan bara är informativ, t.ex. varför en övning ligger i upplägget, vad den tränar, hur den loggas eller vilka risker den har: svara bara på det, använd exerciseLibrary som facit, ingen uppmaning att ändra behövs. Om frågan eller besvärsbeskrivningen är tvetydig — t.ex. oklart vilken rörelse som gör ont, hur länge det pågått, eller vad som redan provats — ställ en kort, konkret följdfråga istället för att gissa. En fråga i taget. ${HEALTH_NOTES_PRECEDENCE_RULE} Detsamma gäller om användarens senaste meddelande eller existingPreferences säger att något är bättre — då kan övningar som tidigare valts bort eller anpassats av den anledningen läggas tillbaka. Skriv som en coach, inte som support. Håll svaret kort och konkret.`;
...
export function buildCoachWorkoutReviewPromptPayload(
  ...
  return {
    ...
    //
    // Här stod också "Om smärta, failure eller avbrott: lyft det som ett klokt
    // beslut, aldrig som ett misslyckande." Samma sak som röstbriefens
    // beslutsrad, struken 2026-09-15. Mätt 2026-09-16 på ett pass med
    // axelhugg och avbrott: "rätt beslut" i 4 av 6 med raden och 2 av 6 utan,
    // och ingen negativ ram någon gång. Modellen berömmer ett klokt stopp själv.
    instruction:
